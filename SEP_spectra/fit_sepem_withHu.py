...
#
  ...
  for ir,row in listrow.iterrows():
    temp_band = lBand(R_fit,row["J0"],row["gamma1"],row["gamma2"],row["R0"])
  ...
  #print (datemin,datemax,band_popt)
  ...
  if plot:
    ...
    plt.savefig("Storm_sepem_hulims/"+str(gle)+".png",bbox_inches="tight")
    plt.close()
...
df_hu.to_csv("band_sepem_withHUlims_fit.csv")
  #break
